...
$("#repairPrice").oninput=calculate;$("#paidPrice").oninput=calculate;
function openAdd(){openPhone()}
function openPhone(id=null){$("#phoneForm").reset();$("#editingId").value="";$("#phoneModalTitle").textContent="إضافة هاتف";$("#status").value="under";$("#createdAtText").value=formatDate(new Date());if(id){let r=phones[id];$("#phoneModalTitle").textContent="تعديل الهاتف";$("#editingId").value=id;$("#deviceName").value=r.deviceName||"";$("#clientName").value=r.clientName||"";$("#clientPhone").value=r.clientPhone||"";$("#problem").value=r.problem||"";$("#repairPrice").value=r.price||"";$("#paidPrice").value=r.paid||"";$("#remainPrice").value=r.remaining||"";$("#notes").value=r.notes||"";$("#status").value=r.status||"under";$("#createdAtText").value=r.createdAtText||formatDate(new Date(r.createdAt||Date.now()))}$("#phoneModal").classList.remove("hidden")}
function closePhoneModal(){$("#phoneModal").classList.add("hidden")}
$("#phoneForm").onsubmit=async e=>{e.preventDefault();calculate();let id=$("#editingId").value||newId(),old=phones[id]||{},now=new Date();let data={deviceName:$("#deviceName").value.trim(),clientName:$("#clientName").value.trim(),clientPhone:$("#clientPhone").value.trim(),problem:$("#problem").value.trim(),price:num($("#repairPrice").value),paid:num($("#paidPrice").value),remaining:num($("#remainPrice").value),notes:$("#notes").value.trim(),status:$("#status").value,createdAt:old.createdAt||now.getTime(),createdAtText:$("#createdAtText").value.trim()||formatDate(now),updatedAt:Date.now()};if(!data.deviceName||!data.clientName)return toast("اكتب اسم الهاتف والزبون");await root().child("phones").child(id).set(data);closePhoneModal();goPage("phonesPage","قائمة الهواتف");toast("تم الحفظ")}
function formatDate(d){let pad=n=>String(n).padStart(2,"0");return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}`}
function renderPhones(){let q=($("#searchPhone")?.value||"").toLowerCase(),sf=$("#filterStatus")?.value||"all";let list=arr().filter(([id,r])=>(sf==="all"||r.status===sf)&&`${r.deviceName} ${r.clientName} ${r.clientPhone}`.toLowerCase().includes(q));$("#phonesList").innerHTML=list.slice(0,8).map(([id,r])=>`<article class="phoneCard"><div class="phoneTop"><div><h3>${esc(r.deviceName)}</h3><p>${esc(r.clientName)} - ${esc(r.clientPhone)}</p><span class="badge ${r.status}">${statusText[r.status]}</span></div><b class="price">${money(r.remaining)}</b></div><p>${esc(r.problem)}</p><p>${esc(r.createdAtText||"")}</p><div class="actions"><button onclick="openPhone('${id}')">تعديل</button><button onclick="quickStatus('${id}')">حالة</button><button onclick="showInvoice('${id}')">فاتورة</button><button onclick="delPhone('${id}')">حذف</button></div></article>`).join("")||"<p>لا توجد هواتف</p>"}
$("#searchPhone").oninput=renderPhones;$("#filterStatus").onchange=renderPhones;
function quickStatus(id){let v=prompt("under=تحت الصيانة\ndone=تم الاكتمال\ndelivered=تم التسليم\nrejected=مرفوض",phones[id].status);if(v&&statusText[v])root().child("phones").child(id).update({status:v,updatedAt:Date.now()})}
function delPhone(id){if(confirm("حذف الهاتف؟"))root().child("phones").child(id).remove()}
function renderStats(){let total=0,paid=0,rem=0,count=0,under=0,done=0,del=0,rej=0;Object.values(phones).forEach(r=>{count++;total+=Number(r.price||0);paid+=Number(r.paid||0);rem+=Number(r.remaining||0);if(r.status==="under")under++;if(r.status==="done")done++;if(r.status==="delivered")del++;if(r.status==="rejected")rej++});$("#statsBox").innerHTML=`<div class="statCard"><span>عدد الهواتف</span><b>${count}</b></div><div class="statCard"><span>المبلغ الكلي</span><b>${money(total)}</b></div><div class="statCard"><span>المستلم</span><b>${money(paid)}</b></div><div class="statCard"><span>الباقي دين</span><b>${money(rem)}</b></div><div class="statCard"><span>تحت الصيانة</span><b>${under}</b></div><div class="statCard"><span>مكتملة</span><b>${done}</b></div><div class="statCard"><span>تم التسليم</span><b>${del}</b></div><div class="statCard"><span>مرفوضة</span><b>${rej}</b></div>`}
function trackUrl(id){return `${location.origin}${location.pathname}?uid=${uid()}&id=${id}&track=1`}
function showInvoice(id){let r=phones[id];lastInvoice=id;let qr=`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(trackUrl(id))}`,terms=(profile.terms||[]).map((t,i)=>`<div>${i+1}. ${esc(t)}</div>`).join("")||"<div>لا توجد شروط</div>";$("#invoicePreview").innerHTML=`<div class="invoice"><div class="invHead"><div><h2>${esc(profile.shopName||"RepairOS")}</h2><small>${esc(profile.phone||"")} ${esc(profile.address||"")}</small></div><b>#${id.slice(-6).toUpperCase()}</b></div><div class="invBody"><div><div class="invBox"><div class="row"><b>الزبون</b><span>${esc(r.clientName)}</span></div><div class="row"><b>رقم الزبون</b><span>${esc(r.clientPhone)}</span></div><div class="row"><b>الهاتف</b><span>${esc(r.deviceName)}</span></div><div class="row"><b>المشكلة</b><span>${esc(r.problem)}</span></div><div class="row"><b>الحالة</b><span>${statusText[r.status]}</span></div><div class="row"><b>التاريخ</b><span>${esc(r.createdAtText)}</span></div><div class="row"><b>ملاحظات</b><span>${esc(r.notes||"-")}</span></div></div><div class="invBox"><div class="row"><b>السعر</b><span>${money(r.price)}</span></div><div class="row"><b>الواصل</b><span>${money(r.paid)}</span></div><div class="row"><b>الباقي</b><span>${money(r.remaining)}</span></div></div></div><div class="invBox qr"><img src="${qr}"><p>باركود متابعة حالة الهاتف</p></div></div><div class="invFoot"><b>الشروط:</b>${terms}</div></div>`;$("#invoiceModal").classList.remove("hidden")}
...
function goPage(id,title){$$(".page").forEach(p=>p.classList.remove("show"));$("#"+id).classList.add("show");$("#topTitle").textContent=title;$("#backBtn").classList.toggle("hidden",id==="homePage")}
$("#backBtn").onclick=()=>goPage("homePage","الرئيسية");
function openCompany(){fillSettings();$("#companyModal").classList.remove("hidden")}function closeCompany(){$("#companyModal").classList.add("hidden")}
function fillSettings(){$("#setCompany").value=profile.shopName||"";$("#setOwner").value=profile.ownerName||"";$("#setCompanyPhone").value=profile.phone||"";$("#setAddress").value=profile.address||"";$("#setLogo").value=profile.logo||"";renderTerms()}
$("#companyForm").onsubmit=async e=>{e.preventDefault();await root().child("profile").update({shopName:$("#setCompany").value.trim(),ownerName:$("#setOwner").value.trim(),phone:$("#setCompanyPhone").value.trim(),address:$("#setAddress").value.trim(),logo:$("#setLogo").value.trim()});closeCompany();toast("تم حفظ تفاصيل الشركة")}
function openThemes(){$("#themesModal").classList.remove("hidden")}function closeThemes(){$("#themesModal").classList.add("hidden")}function setTheme(t){document.body.dataset.theme=t;localStorage.theme=t;if(currentUser)root().child("profile/theme").set(t);closeThemes()}
function openTerms(){renderTerms();$("#termsModal").classList.remove("hidden")}function closeTerms(){$("#termsModal").classList.add("hidden")}
